...
tendon removals. So if the cascade already removed it, drop the explicit
...
- The terrain XML defines both a skybox texture *and* a 2D texture with a
  material -- both must survive the strip for the skybox to render. This is not critical, mostly a visual artifact, but helpful for ensuring the pipeline is working as intended.
...
### Initial camera view is off center or weirdly rotated
